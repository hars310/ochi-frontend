...
//   const [scaleVal,setscaleVal] = useState(0);
  ...
  // Calculate the scale value based on loadingCounter
  let scaleValue = loadingCounter / 79;
  

  return (
    <motion.div
    //   style={{
    //     scale: scaleVal, // Scale the component
    //   }}
      className="bg-zinc-900 rounded-2xl w-full px-1 pb-16 py-2  overflow-hidden flex flex-col"
      initial={{ scale: .9 }}
      animate={{ scale: scaleValue }}
      transition={{ delay: 0.3, duration: .2 }}
    >
      <div className="logo text-white px-16 py-7 mb-2">
      <svg
          width="60"
          height="30"
          viewBox="0 0 72 30"
      ...
      <div className="textData py-4 px-20">
        {["WE CREATE", "EYE OPENING", "PRESENTATIONS"].map((item, index) => (
          <div key={index} className="masker">
            <h1 className="flex items-center text-[6vw] leading-[5.5vw] tracking-tighter font-semibold">
              {item}
            </h1>
          </div>
        ))}
        <div className="flex justify-between px-4">
          <div className="text-lg mt-6 px-4">Loading:</div>
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          ...
          >
            {loadingCounter}% {/* Display percentage */}
          </motion.div>
        </div>
      </div>
    </motion.div>
